			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back-end/RepairForm/select_page.jsp");
				...
				String url ="/back-end/RepairForm/listOneRepairForm.jsp";
...
//===============================    ↓ 前台單一查詢     ======================================		
		if ("getOne_For_Displayf".equals(action)) {  // 來自select_page.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req
							.getRequestDispatcher("/front-end/RepairForm/select_page.jsp");
					failureView.forward(req, res);
					return;//程式中斷
				}
				...
				/**************************3.查詢完成,準備轉交(Send the Success view)***************/
				req.setAttribute("repairformVO", repairformVO); // 資料庫取出的empVO物件,存入req
				String url ="/front-end/RepairForm/listOneRepairForm.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url); // 成功轉交 listOneEmp.jsp
				successView.forward(req, res);
...
//===============================  ↑ 前台單一查詢     ======================================			
		...
		if ("getOne_For_Update".equals(action)) {// 來自listAllEmp.jsp的請求
			...
			try {
				...
				/****************************3.查詢完成,準備轉交(Send the Success view)************/			
				req.setAttribute("repairformVO", repairformVO);  // 資料庫取出的empVO物件,存入req
				String url ="/back-end/RepairForm/update_RepairForm_input.jsp";
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/back-end/RepairForm/listAllRepairForm.jsp");
				failureView.forward(req, res);
			}
		}
...
//==========================================================================================
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back-end/RepairForm/update_RepairForm_input.jsp");
					failureView.forward(req, res);
					return; //程式中斷
				}
        ...
        if ("insert".equals(action)) { // 來自後台addEmp.jsp的 
...
//				
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("repairformVO", repairformVO);// 含有輸入格式錯誤的empVO物件,也存入req
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back-end/RepairForm/addRepairForm.jsp");
					failureView.forward(req, res);
					return;
				}
				...
				String url ="/back-end/RepairForm/listAllRepairForm.jsp";
...
//				
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("repairformVO", repairformVO); // 含有輸入格式錯誤的empVO物件,也存入req
					RequestDispatcher failureView = req
							.getRequestDispatcher("/front-end/RepairForm/addRepairForm.jsp");
					failureView.forward(req, res);
					return;
				}
				...
				/**************************3.新增完成,準備轉交(Send the Success view)************/
				String url ="/front-end/RepairForm/select_page.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url); // 新增成功後轉交listAllEmp.jsp
				successView.forward(req, res);				
